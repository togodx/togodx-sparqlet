...
({data}) => {
  ...
  const nodePrefix = "http://id.nlm.nih.gov/mesh/";
  let tree = [
    {
      id: "root",
      label: "Diseases",
  ...
  data.results.bindings.forEach(d => {
    tree.push({
      id: (d.tree_child == undefined ? d.mesh_id.value.replace(idPrefix, "") : d.node.value.replace(nodePrefix, "")), 
      label: d.mesh_label.value,
      leaf: (d.tree_child == undefined ? true : false),
      parent: (d.parent == undefined ? "root" :  d.parent.value.replace(nodePrefix, ""))
    });
  });
  return tree;
};
